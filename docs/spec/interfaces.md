...
| `inject` | `{"op":"inject","handle":…,"sandbox_identity":{"sandbox_id":…,"attestation":{"alg":"ed25519","payload":…,"signature":…}?},"mode":"env\|proxy"}` | proxy: `{"ok":true,"delivery":"proxy","credential":…,"binding":{…}}` · env: `{"ok":true,"delivery":"env","credential":…,"var_name":…,"wiped_at":<unix_secs>}` · expired: `{"error":{"code":"handle_expired",...}}` · rotated: `{"error":{"code":"handle_invalidated",...}}` (secret rotated after resolve — ADR-004) · tampered ciphertext: `{"error":{"code":"decrypt_failed",...}}` (AES-256-GCM tag check — ADR-005) · attestation (only when a trust root is configured, ADR-010): missing member → `{"error":{"code":"attestation_missing",...}}`, bad/tampered/wrong-key/id-mismatch → `{"error":{"code":"attestation_invalid",...}}` — verified **before** any handle mutation, so a rejected attestation never consumes or binds the handle |
