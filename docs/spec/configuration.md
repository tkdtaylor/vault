...
| `--secret-backend` | `serve` | string (`mock` \| `alt-mock`) | — (absent → AES at-rest / persistent store) | no | **Opt-in** cloud secret-manager store backend core (ADR-007). Present → the store backend is a `SecretManagerBackend` over the selected `SecretManagerClient` (via the `secret_manager::make_client` drop-in registry); the value re-materialises via the client at `inject` instead of decrypting a local AES ciphertext, and a failed remote op is fail-closed (`backend_unavailable`). This L2 core ships only the **mock** adapters (`mock` / `alt-mock`); the real per-cloud adapters (AWS/GCP/Azure) are task 012 (feature-gated). An unknown name **refuses to start**. When set it **supersedes** `--store-path` (the remote holds the value; nothing persists locally) |
