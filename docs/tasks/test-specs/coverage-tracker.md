...
| 007 | Persistent encrypted-on-disk store (opt-in, key off disk, handles never persist) | `007-persistent-encrypted-store-test-spec.md` | TC-001…TC-010 (planned) | ❌ | Pending — backlog. Implements [ADR-008](../architecture/decisions/008-persistent-encrypted-store.md) (Accepted; executor writes NO new ADR). Opt-in `--store-path`/`VAULT_STORE_PATH` `StoreFile` layer (orthogonal to the `StoreBackend` seam): atomic `0600` JSON of `EncryptedValue`+metadata via a `StoredRecord` DTO; key never on disk, handles never persist (restart ⇒ `unknown_handle`), refuse-to-start on corrupt file, new `store_persist_failed`. Highest local level: L6 (in-process drop+reload round-trip + live `serve --store-path` restart smoke); L5 = `cargo build && cargo test` green (48 prior tests stay green). Not started. |
