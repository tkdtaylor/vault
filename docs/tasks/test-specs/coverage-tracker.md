...
| 005 | Vault HTTP API read surface (zero-knowledge, read-only, loopback) | `005-vault-http-api-read-surface-test-spec.md` | TC-001…TC-011 | ❌ | pending — backlog (ADR-006; opt-in `--http-addr`, loopback-only, `GET /v1/sys/health` + `GET /v1/secret/data/:path`→`resolve`→handle in KV-v2 envelope never the value, fail-closed 405/404/400, `tiny_http` 0.12 dep-scan gate) |
